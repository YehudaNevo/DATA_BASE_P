create or replace package body DB_functions is
  ...
  restaurantId in integer, k_institution_id in integer, 
  k_level in string, price in integer
) return integer is cursor supervisor is 
select 
  ks_id 
from 
  (
      ...
      (
        (
          select 
            ks_name, 
            ks_id, 
            count(kc_id) as certs_nember 
          from 
            kosher_certificate natural 
            join kosher_supervisor 
          group by 
            ks_name, 
            ks_id
        ) natural 
        join (kosher_instetution)
      ) 
    where 
      ki_id = k_institution_id 
    order by 
      certs_nember asc
  ) 
where 
...
SELECT 
  dbms_random.value(100000, 999999) into certId 
FROM 
  dual;
open supervisor;
FETCH supervisor INTO supervisorId;
...
insert into kosher_certificate (
  kc_id, strat_date, end_date, k_level, 
  ks_id, price_pm
) 
values 
  (
    certId, current_date, endDate, k_level, 
    supervisorId, price
  );
